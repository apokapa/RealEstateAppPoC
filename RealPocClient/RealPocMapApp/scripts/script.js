    ...
    function onDataLoad(res) {
        setTimeout(function () {
            ...
            for (var item of model) {
                var parseModel = {
                    link: "http://realpocspa.azurewebsites.net/" + Functions.removeAllSpaces(item.Title) + "/" + item.OfferId + "/" + item.OfferCategory[0],
                ...
                var text = NZapp.createView("views/offer-view/component.html", parseModel, onNewViewLoaded, onNewViewRendered);
